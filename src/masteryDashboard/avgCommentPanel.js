...
export function buildAvgCommentToggleHtml(rawComments) {
    ...
    return `
        <button id="${TOGGLE_ID}"
            ...
            aria-expanded="false"
            title="View comments"
            style="background:none; border:none; cursor:pointer; padding:4px 2px; color:#555; font-size:1.1rem; line-height:1; flex-shrink:0; border-radius:4px;">
            💬
        </button>
    `;
}
...
export function buildAvgCommentPanelHtml(rawComments) {
    ...
    const hasMore = rawComments.length > MAX_VISIBLE;
    ...
    const commentRows = comments.map(c => {
        ...
        return `
            <div style="padding:8px 0; border-bottom:1px solid #f0f0f0;">
                <div style="${FONT} font-size:0.75rem; color:#888; margin-bottom:2px; line-height:1.4; -webkit-font-smoothing:antialiased;">
                    ${escapeHtml(c.author_name || 'Teacher')}${date ? ` · ${date}` : ''}
                </div>
                <div style="${FONT} font-size:0.85rem; color:#333; line-height:1.5; -webkit-font-smoothing:antialiased;">
    ...
    return `
        <div id="${PANEL_ID}" style="display:none; margin-top:8px;">
            <div style="${FONT} font-size:0.72rem; font-weight:600; color:#666; margin-bottom:4px; letter-spacing:0.04em; text-transform:uppercase; -webkit-font-smoothing:antialiased;">
                Teacher Comments${hasMore ? ` (showing ${MAX_VISIBLE} of ${rawComments.length})` : ''}
            </div>
...
export function initAvgCommentPanel() {
    ...
    toggle.addEventListener('click', e => {
        ...
        toggle.style.color = isOpen ? '#555' : '#0770A3';
    });
}
